...
class MatNet(nn.Module):
    def __init__(self):
        ...
        self.ths_v = [
            nn.Parameter(
                Variable(
                    torch.tensor(ths_initial, dtype=datatype, device=device),
                    requires_grad=True,
                )
            )
        ...
        self.step = [
            nn.Parameter(
                Variable(
                    torch.tensor(step_initial, dtype=datatype, device=device),
                    requires_grad=True,
                )
            )
            for t in range(maxIt)
        ...
        self.ths_backup = [
            torch.tensor(0.0, dtype=datatype, device=device) for t in range(maxIt)
        ]

    def thre(self, inputs, threshold):
        out = torch.sign(inputs) * torch.max(
            torch.abs(inputs) - threshold,
            torch.zeros([1, 1], dtype=datatype, device=device),
        )
        return out
...
for i in range(maxIt):
    optimizer = optim.SGD(
        {net.ths_v[i]}, lr=lr_fac * ths_initial / 5000.0
    )  # optimizer for each layer
    optimizer.add_param_group(
        {"params": [net.step[i]], "lr": lr_fac * 0.1}
...
for stage in range(maxIt):  # in k-th stage, we train the k-th layer
    ...
    if stage > 6:
        Nepoches_full = 500
    if stage > 0:
        optimizers[stage].param_groups[0]["lr"] = (
            net.ths_v[stage - 1].data * lr_fac / 5000.0
        )
